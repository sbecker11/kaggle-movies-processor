...
-- \c database
-- CREATE SCHEMA kaggle_schema;
-- \c database
CREATE TABLE kaggle_schema.movies (
    id INT PRIMARY KEY,
    genres TEXT,
    revenue NUMERIC,
    budget NUMERIC,
...
COPY kaggle_schema.movies (id, budget, genres, release_date, revenue, spoken_languages)
...
SELECT release_date FROM kaggle_schema.raw_movies 
LIMIT 5; -- '\d{4}-\d{2}-\d{2}'

-- null release_date = 0
SELECT COUNT(*) FROM kaggle_schema.raw_movies 
...
select spoken_languages from kaggle_schema.raw_movies
